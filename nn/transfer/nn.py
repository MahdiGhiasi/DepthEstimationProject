...
TRAIN_EPOCH_COUNT_ON_EACH_SUBSET = 15
...
def train_model(model_input_file, model_output_file, x_eval_02, x_eval_03, y_eval, test_sample_x_02, test_sample_x_03, test_sample_index, logdir):
    ...
    # densenet201 = keras.applications.densenet.DenseNet201(include_top=False, 
    #     weights='imagenet', 
    #     input_tensor=None, 
    ...
    # pretrained_model.add(densenet201)
    # pretrained_model.add(Flatten())
    # pretrained_model.summary()

    # x_train_02_features = pretrained_model.predict(x_train_02)
    # x_train_03_features = pretrained_model.predict(x_train_03)

    # x_eval_02_features = pretrained_model.predict(x_eval_02)
    # x_eval_03_features = pretrained_model.predict(x_eval_03)

    input1 = Input(shape=(INPUT_HEIGHT, INPUT_WIDTH, 3))


    if len(model_input_file) == 0:        
        densenet_1 = keras.applications.densenet.DenseNet201(include_top=False, 
            weights='imagenet', 
            input_tensor=None, 
        ...
        densenet_2 = keras.applications.densenet.DenseNet201(include_top=False, 
            weights='imagenet', 
            input_tensor=None, 
            input_shape=(INPUT_HEIGHT, INPUT_WIDTH, 3), 
            pooling='avg')
...
if __name__ == '__main__':
    ...
    while True:
        ...
        #process = Process(target=train_model, args=[input_file, output_file, x_eval_02, x_eval_03, y_eval, test_sample_x_02, test_sample_x_03, counter, logdir])
        #process.start()
        #process.join()
